...
var_name_list = []
domain_list = []
pred_constraints_dict = {} # dictionary
max_time = 0
file_line_count = 0
...
#	constraints
class CSP:
	def __init__(self, variables, domain, constraints, deadline, pred_constraints):
		...
		self.pred_constraints = pred_constraints # list of 
	...
	def add_assignment(self, var_name, value, cons_time):
		for var in self.variables:
			if var.name == var_name:
	...
	def print_assignments(self):
		for var in self.variables:
			print("{}[{}]	= {} @ t={}".format(var.name, var.length, var.assignment, var.time))

	def pretty_print_assignments(self):
		timeline = []
		for d in self.domain:
			timeline.append([' '] * deadline)
		
		# Format/Print header
		header = range(deadline)
		sys.stdout.write('   |')
		for i in header:
			t = i + 1
			if t == 1:
		...
		for var in self.variables:
			if var.assignment != '?':
				for i in range(deadline):
					if i >= (var.time-1) and i < var.time+(var.length-1):
						timeline[self.get_index(var.assignment)][i] = var.name

		# Print timeline
		domain_i = 0
		for t in timeline:
			print("{}: {}".format(self.domain[domain_i],t))
			domain_i += 1
	...
	while line:
		...
		if str.splitlines(line[0:5]) == ['#####']:
			...
			if file_section_count == 1: # variables
				variable_list.append(Variable(line_info[0], int(line_info[1]), []))
				var_name_list.append(line_info[0])
				pred_constraints_dict[line_info[0]] = []
				max_time += int(line_info[1])
				...
				# Also take the time to initialize the time domain of each variable
				for variable in variable_list:
					variable.t_domain = range(max_time+1)[1:]
			...
			elif file_section_count == 9: # predecessor constraints
				pred_constraints_dict[line_info[0]] = line_info[1:]
		# Read next line
		line = fp.readline()
...
csp_global = CSP(variable_list, domain_list, constraint_list, deadline, pred_constraints_dict)
...
def order_domain_values(var_name, csp):
	...
	for val_score in value_scores:
		...
		csp_copy.add_assignment(var_name, val_score[0], -1) # adds our testing assignment to the csp
		# csp_copy.print_assignments()
...
# Assuming variable is assigned
def get_end_time(var_name, csp):
	for var in csp.variables:
		if var.name == var_name:
			# print('vartime={}'.format(type(var.time)))
			# print('varlength={}'.format(type(var.length)))
			return var.time + (int(var.length) - 1) # we spend 1 delta at the assigned time
...
def is_consistent(var_name, value, csp):
	...
	## Deadline constraint
	# @todo Need to re-do beacuse this is only best case where we have no depedencies
	...
	# Deadline constraint version 2.0
	...
	for pred in csp.pred_constraints[var_name]:
		if not is_assigned(pred, csp): # If predecessor not assigned not consistent
			...
			if earliest_start_time < get_end_time(pred, csp):
				earliest_start_time = get_end_time(pred, csp) + 1

	# Resources
	for var in csp.variables:
		if var.assignment == value: # assigned to same resource
			if earliest_start_time < get_end_time(var.name, csp):
				earliest_start_time = get_end_time(var.name, csp) + 1

	# Return earliest potential time
	for var in csp.variables:
		if var.name == var_name:
			if (earliest_start_time + 1 + (int(var.length) - 1)) <= deadline + 1:
				# print("time={}".format(earliest_start_time))
				return earliest_start_time

	# No consistent times, so return 0
	return 0
...
def recursive_backtracking(csp):
	...
	for value in order_domain_values(var_name, csp):
		cons_time = is_consistent(var_name, value, csp)
		if cons_time:
			print("{}=={} @ t={} is consistent, adding to assignment".format(var_name, value, cons_time))
			csp.add_assignment(var_name, value, cons_time)
